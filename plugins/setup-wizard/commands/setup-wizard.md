...
23. Repetitive workflows worth turning into slash commands?
    Examples: `/deploy`, `/test-all`, `/new-migration`, `/review-pr`.
24. Specialized subagents worth creating?
...
Automation:
  .claude/commands/<n>.md             — one per Q23
  .claude/agents/<n>.md               — one per Q24
...
### 4.5 · Slash commands (`.claude/commands/<n>.md`)

For each Q23 entry, generate a scaffold using `templates/command.md.template`.
Ask the user for a one-line description per command before writing.

### 4.6 · Subagents (`.claude/agents/<n>.md`)

For each Q24 entry, generate from `templates/agent.md.template`.
Each agent gets YAML frontmatter with `name`, `description`, `tools` (restricted).
...
> 3. Run `/` to see your new slash commands
